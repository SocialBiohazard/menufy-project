    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardTitle className="text-center">Customer workspace created</CardTitle>
        </CardHeader>
        <CardContent className="mx-auto max-w-2xl space-y-5 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            ...
            <Button nativeButton={false} render={<a href={state.activationPath} target="_blank" rel="noreferrer" />}>
              <ExternalLink className="size-4" /> Open activation
            </Button>
        ...
        <form action={action} className="space-y-6">
          ...
          {step === 1 && (
            <div className="space-y-3">
              <div><h2 className="font-medium">Assign restaurant locations</h2><p className="text-sm text-muted-foreground">Choose every location this customer should own initially.</p></div>
              ...
              {!restaurants.length && <p className="rounded-md border border-dashed p-6 text-center text-muted-foreground">Every restaurant is already assigned.</p>}
            </div>
          )}
          ...
          {step === 2 && (
            <div className="grid gap-5 sm:grid-cols-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="onboarding-location-limit">Location limit</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="onboarding-storage">Storage limit (MB)</Label>
              ...
              <p className="text-sm text-muted-foreground sm:col-span-3">These limits stay operator-only and can be changed later.</p>
            </div>
          )}

          {step === 3 && (
            <div className="space-y-4">
              <h2 className="font-medium">Review customer workspace</h2>
              <dl className="grid gap-3 rounded-md border p-4 text-sm sm:grid-cols-2">
                <div><dt className="text-muted-foreground">Customer</dt><dd className="font-medium">{accountName}</dd></div>
                <div><dt className="text-muted-foreground">Owner</dt><dd className="font-medium">{email}</dd></div>
                <div><dt className="text-muted-foreground">Locations</dt><dd className="font-medium">{selectedRestaurants.map((restaurant) => restaurant.businessName).join(", ")}</dd></div>
                <div><dt className="text-muted-foreground">Access</dt><dd className="font-medium">{plan.toLowerCase()} · {maxRestaurants} locations · {maxStorageMb} MB</dd></div>
              </dl>
              <p className="text-sm text-muted-foreground">Creating the workspace assigns these restaurants and generates a one-time activation link. No email is sent automatically yet.</p>
            </div>
          )}
