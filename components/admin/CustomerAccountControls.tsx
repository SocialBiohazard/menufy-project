...
export function CustomerAccountSettings({
  ...
  return (
    <form action={action} className="grid gap-3 rounded-md border p-3 sm:grid-cols-4">
      ...
      <div className="space-y-1"><Label>Location limit</Label><Input name="maxRestaurants" type="number" min={1} defaultValue={account.maxRestaurants} /></div>
      <div className="space-y-1"><Label>Storage (MB)</Label><Input name="maxStorageMb" type="number" min={100} defaultValue={account.maxStorageMb} /></div>
      <div className="space-y-1">
        <Label>Status</Label>
      ...
      <Button type="submit" size="sm" disabled={pending} className="sm:col-span-4 sm:justify-self-end">
        {pending ? "Saving…" : "Save account limits"}
      </Button>
    </form>
  );
}
...
export function AssignRestaurantForm({
  ...
  return (
    <form action={action} className="grid gap-3 rounded-lg border p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
      ...
      <Button type="submit" disabled={pending || !accounts.length || !restaurants.length}>{pending ? "Assigning…" : "Assign location"}</Button>
